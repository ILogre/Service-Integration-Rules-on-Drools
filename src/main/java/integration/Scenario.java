...
public class Scenario {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("[Scenario - Step "+ step++ +"] : Declare a Catalog");
		bus.handle(new DeclareCatalogMsg(catalogName));

		System.out.println("[Scenario - Step "+ step++ +"]: Declare a Dashboard");
		...
		System.out.println("[Scenario - Step "+ step++ +"] : Build an branch of the building hierarchy");
		...
		System.out.println("[Scenario - Step "+ step++ +"] : Sketch a new scheme of stored data");
		...
		System.out.println("[Scenario - Step "+ step +"] : Expose a new periodic sensor");
		bus.handle(new RecordPeriodicSensorMsg(catalogName, "Office443",
				"tempin_443", "SC_Temperature", 2));
		System.out.println("[Scenario - Step "+ step++ +"bis] : Expose a new periodic sensor");
		bus.handle(new RecordPeriodicSensorMsg(catalogName, "Office443",
				"tempout_443", "SC_Temperature", 300));

		System.out.println("[Scenario - Step "+ step++ +"] : Check that the Catalog model is currently not validated");
		IsValidatedCatalogMsg failed = new IsValidatedCatalogMsg(catalogName);
		bus.handle(failed);
		IsValidatedCatalogAsw ansFailed = (IsValidatedCatalogAsw) mailbox.get(failed);
		System.out.println("Result : Catalog validation status = "+ansFailed.isValidated());

		System.out.println("[Scenario - Step "+ step++ +"] : Validate and persist the Catalog model");
		bus.handle(new ValidateAndPersistCatalogMsg(catalogName));

		System.out.println("[Scenario - Step "+ step++ +"] : Check that the Catalog model is now validated");
		IsValidatedCatalogMsg success = new IsValidatedCatalogMsg(catalogName);
		bus.handle(success);
		IsValidatedCatalogAsw ansSuccess = (IsValidatedCatalogAsw) mailbox.get(success);
		System.out.println("Result : Catalog validation status = "+ansSuccess.isValidated());

		System.out.println("[Scenario - Step "+ step++ +"] : Add a visu to the dashboard");
		bus.handle(new CharacterizeVisuMsg(dashboardName, "OfficeCooling", "Comparison", "Over_time"));

		System.out.println("[Scenario - Step "+ step++ +"] : Plug a data on this visualization");
